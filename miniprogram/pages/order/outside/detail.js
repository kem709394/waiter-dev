...
Page({
  ...
  getData(id) {
    ...
    db.collection('outside_order').where({
      ...
      if (detail.state < 20) {
        ...
        if (detail.payable_money && detail.payment_money==0) {
          operate.payment = true
        }
      }
      if (detail.state == 13) {
        self.getDelivery(detail.delivery_id)
      }
      self.setData({
        detail: detail,
        operate: operate
  ...
  getDelivery(id) {
    let self = this
    db.collection('delivery').doc(id).get().then(res=>{
      let delivery = res.data
      delivery.time = moment(delivery.delivery_time).format('YYYY-MM-DD HH:mm')
      app.tempStaffReadyCallback = () => {
        self.setData({
          temp_staff: app.globalData.temp_staff
        })
      }
      app.syncTempStaff([delivery.delivery_sid])
      self.setData({
        delivery: delivery
      })
    })
