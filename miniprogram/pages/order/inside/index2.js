...
Page({
  ...
  onShow() {
    ...
    app.location().then(res=>{
      ...
      if (distance > self.data.config.inside.reserve.use_distance) {
        wx.showToast({
          icon: 'none',
          title: '您的位置超出服务范围',
          duration: 2000
  ...
  onShareAppMessage() {
    ...
    return {
      title: base.name + '-点餐',
      path: '/pages/order/inside/index2',
  ...
  affirmOrder() {
    ...
    self.getDistance().then(res => {
      if (res > self.data.config.inside.reserve.use_distance) {
        wx.showToast({
          icon: 'none',
          title: '您的位置超出服务范围',
          duration: 2000
        ...
        if (self.data.granted) {
          if (self.data.order.total.money > 0) {
            wx.navigateTo({
              url: 'affirm2'
            })
          }
