...
Page({
  ...
  onLoad(options) {
    ...
    if (privilege.includes('column-update')) buttons.push({text: '编辑'})
    if (privilege.includes('column-remove')) buttons.push({text: '删除',type: 'warn'})
    self.setData({
      title: options.title,
      buttons: buttons,
      privilege: privilege
  ...
  init() {
    ...
    db.collection('column').aggregate().match({
      is_deleted: false
    }).group({
      _id: '$visible',
      count: $.sum(1)
  ...
  load() {
    ...
    if (!self.data.finish && !self.data.loading) {
      ...
      let aggregate = db.collection('column').aggregate().match({
        is_deleted: false
      })
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          db.collection('column').doc(item._id).update({
            data: {
              is_deleted: true,
