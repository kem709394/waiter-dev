...
Page({
  ...
  onLoad(options) {
    ...
    if (privilege.includes('table-update')) buttons.push({
      text: '编辑'
    })
    if (privilege.includes('table-remove')) buttons.push({
      text: '删除',
      type: 'warn'
    })
    self.setData({
  ...
  load() {
    ...
    if (!self.data.finish && !self.data.loading) {
      ...
      let aggregate = db.collection('table').aggregate().match({
        is_deleted: false
      })
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          db.collection('table').doc(item._id).update({
            data: {
              is_deleted: true,
