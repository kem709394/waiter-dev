...
Page({
  ...
  onLoad(options) {
    ...
    self.setData({
      ...
      'form.full_name': detail.full_name,
      'form.gender': detail.gender,
      'form.mobile': detail.mobile,
      'form.openid': detail.openid,
      'form.position': detail.position,
      'form.privilege': tools.objCopy(detail.privilege),
      'form.state': detail.state,
      'form.note': detail.note,
      'form.priority': detail.priority,
      'models.full_name': detail.full_name,
      'models.mobile': detail.mobile,
      'models.position': detail.position,
  ...
  submitForm(e) {
    ...
    self.selectComponent('#form').validate((valid, errors) => {
      if (!valid) {
        ...
        db.collection('staff').doc(self.data.detail._id).update({
          data: data
        }).then(res=>{
          wx.showToast({
            title: '保存成功',
