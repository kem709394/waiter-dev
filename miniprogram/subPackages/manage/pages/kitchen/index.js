...
Page({
  ...
  onLoad(options) {
    ...
    if (privilege.includes('kitchen-update')) buttons.push({text: '编辑'})
    if (privilege.includes('kitchen-remove')) buttons.push({text: '删除',type: 'warn'})
    self.setData({
      title: options.title,
      buttons: buttons,
      privilege: privilege
  ...
  load() {
    ...
    if (!self.data.finish && !self.data.loading) {
      ...
      db.collection('kitchen').where({
        is_deleted: false
      }).skip(self.data.current).limit(self.data.size).orderBy('priority', 'asc').get().then(res => {
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          db.collection('kitchen').doc(item._id).update({
            data: {
              is_deleted: true,
