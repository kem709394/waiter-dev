...
Page({
  ...
  affirmOrder() {
    ...
    if (self.data.order.total.menu > 0) {
      app.globalData.rt_data.temp_outside_data = self.data.order.list
      wx.navigateBack()
    }
  },
  addOrderAmount(e) {
  ...
  statTotal() {
    ...
    self.data.order.list.forEach(item => {
      if (!item.form.gift) {
        ...
        if (item.form.pack) {
          if (menu.pack_mode == '1') {
            total_pack_fee += menu.pack_fee * item.amount
          } else {
            total_pack_fee += menu.pack_fee
          }
        }
    ...
    self.setData({
      ...
      'order.total.money': total_money + total_pack_fee
    })
    wx.setStorage({
      key: 'temp_outside',
